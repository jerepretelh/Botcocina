...
async function readRequestBody(req: any): Promise<{ prompt?: string; mode?: string }> {
  if (req?.body && typeof req.body === 'object') {
    return req.body as { prompt?: string; mode?: string }
  }

  if (typeof req?.body === 'string') {
    try {
      return JSON.parse(req.body) as { prompt?: string; mode?: string }
    } catch {
      return {}
    }
  }
  ...
  if (!text.trim()) return {}
  return JSON.parse(text) as { prompt?: string; mode?: string }
}
