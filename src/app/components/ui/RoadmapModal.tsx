...
export function RoadmapModal({
    ...
    const flatSubSteps = steps.flatMap((step, sIdx) =>
    ...
    const currentFlatIndex = flatSubSteps.findIndex(
            ...
            <div className="bg-slate-900 border border-slate-700 w-full max-w-2xl max-h-[80vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl">
                ...
                <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">Previsualiza los subpasos de tu receta</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                    {flatSubSteps.map((item, flatIdx) => {
                        const isPastSubStep = currentFlatIndex >= 0 && flatIdx < currentFlatIndex;
                        const isCurrentSubStep = flatIdx === currentFlatIndex;
                        const canJump = onJumpToSubStep && !isCurrentSubStep;

                        return (
                            <div key={`${item.stepIndex}-${item.subStepIndex}`} className="relative pl-8">
                                <div
                                    className={`absolute left-0 top-0 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold z-10 transition-colors ${isPastSubStep
                                ...
                                >
                                    {isPastSubStep ? '✓' : flatIdx + 1}
                                </div>

                                {flatIdx < flatSubSteps.length - 1 && (
                                    <div
                                        className={`absolute left-[11px] top-6 w-0.5 h-full -mb-8 transition-colors ${isPastSubStep ? 'bg-emerald-500/30' : 'bg-slate-800'
                                            }`}
                                    />
                                )}
                                ...
                                <button
                                    disabled={!canJump}
                                    onClick={() => onJumpToSubStep?.(item.stepIndex, item.subStepIndex)}
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        {item.sub.isTimer && (
                                            <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded-full flex items-center gap-1 border border-slate-700">
                                                ⏱️ {item.sub.portions[portion] || item.sub.portions[1]}s
                                            </span>
                                        )}
                                    </div>
                                </button>
