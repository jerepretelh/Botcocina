...
// The state block starts with `const [screen, setScreen]` and ends before `const activeRecipeId = selectedRecipe?.id ?? 'arroz';`
const stateBlockRegex = /const \[screen, setScreen\][\s\S]*?(?=const activeRecipeId = selectedRecipe\?\.id \?\? 'arroz';)/;
...
const duplicateComputationsRegex = /const activeRecipeId.*?const batchUsageTips.*?;\n/s;
